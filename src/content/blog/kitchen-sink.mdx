...
Body copy supports **bold**, _italic_, ***bold italic***, ~~strikethrough~~,
...
<Callout type="warn" title="! gotcha">
  ...
  plugin. The `prose-terminal` overrides are intentionally unlayered so they
  win the cascade — see `app/globals.css`.
</Callout>
...
![A minimal terminal illustration in paper and ink](/images/kitchen-sink.png)
...
<Callout type="tip">
  Want a new element style? It only needs to be defined once in
  `prose-terminal` or added to the component map — every post inherits it
  automatically.
</Callout>
